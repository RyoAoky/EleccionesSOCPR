$(document).ready(function () {
  ...
  let tiemval_cermed = document.getElementById("tiemval_cermed");
  let fecvcto_cermed = document.getElementById("fecvcto_cermed");
  tiemval_cermed.disabled = true;
  fecvcto_cermed.disabled = true;
  $('input[name="respuesta1"]').change(function () {
    let estado = $(this).val();
    if (estado === 'S') {
      tiemval_cermed.disabled = false;
      fecvcto_cermed.disabled = false;
    } else {
      tiemval_cermed.disabled = true;
      fecvcto_cermed.disabled = true;
    }
  ...
  $('#ex1 a[data-bs-toggle="tab"]').on('shown.bs.tab', function (e) {
    ...
    const inputsearch = document.getElementById("inputSearchExamen"); // agregado busqueda de examenes
...
document.getElementById("inputSearchExamen").addEventListener("input", InputSearchExamen)

function InputSearchExamen() {
  var input = document.getElementById("inputSearchExamen").value.toString().toLowerCase();
  var table = document.getElementById("tbodyexamen");
  var rows = table.getElementsByTagName("tr");

  for (var i = 0; i < rows.length; i++) {
    let textoConsulta = rows[i].cells[1].textContent.toString().toLowerCase();
    if (textoConsulta.indexOf(input) === -1) {
...
function llenarFormulario(id) {
  var formElements = {
    ...
    cadcermed: document.getElementById('cadcermed')
  };
  ...
  $.ajax({
    ...
    success: function (datos) {
      ...
      $('input[type="radio"][name="estado"][value="' + data.estado + '"]').prop('checked', true);
      $('input[type="radio"][name="respuesta1"][value="' + data.cadcermed + '"]').prop('checked', true);
  ...
  $.ajax({
    ...
    success: function (TipoExamenes) {
      ...
      if (id === "0") {
        fechahoy2("fec_emi");
        CalcularFecVen();
        $('input[type="radio"][name="estado"][value="V"]').prop('checked', true);
        let estado = $('input[type="radio"][name="estado"]');
        estado.trigger('change');
      } else {
        llenarFormulario(id);
...
function Guardar() {
  var camposValidos = validarInputs('nompro');
  if (!camposValidos) {
    return;
  }
  let medapmn = document.getElementById("medapmn").value;
  if (medapmn === "") {
    mensajecentral('error', 'Debe registrar un médico.');
    return;
  }
  ...
  if (datains.length === 0) {
    ...
    var Guardar = document.getElementById("Guardar"); //boton de guardar
    ...
    let cadcermed = $('input[type="radio"][name="respuesta1"]:checked').val();
...
function getmedico() {
  let parametro1 = document.getElementById("medicomodal").value;
