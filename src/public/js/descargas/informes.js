...
document.getElementById("paciente").addEventListener("keydown", function (event) {
  if (event.key === "Enter") {
    getPacientesDescarga();
  }
});
function getPacientesDescarga() {
  ...
  if (selectedValue !== "") {
    ...
    $.ajax({
      ...
      error: function () {
        $("#error-message").text(
          "Se produjo un error al cargar los pacientes citados."
        );
      },
    });
  } else {
  ...
  if (seleccionados.length === 0) {
    ...
    $.ajax({
      ...
      success: function (pruebas) {
        ...
        // Agregar el checkbox "TODOS" antes de entrar al bucle forEach
        examenes.append(`
        <div class="form-inline col-sm-12 form-group mb-1 mt-1">
          <div class="input-group-text"
            ...
            <input class="form-check-input mt-0" type="checkbox" style="border-radius: 0px;" id="CheckTodosExamenes" onclick="CheckTodosExamenes()">
          </div>
          <label class="control-label input-group-text" for="CheckTodosExamenes"
            style="height: 30px;border-top-left-radius: 0px;border-bottom-left-radius: 0px; width:90%; justify-content: left; align-items: center">TODOS</label>
        </div>
      `);

        // Bucle forEach para agregar checkboxes de exámenes
      ...
      error: function () {
        $("#error-message").text(
          "Se produjo un error al cargar los protocolos."
        );
      },
    });
  }
...
function imprimir(iddiv, iddatatableble) {
  ...
  var checkboxes = div.querySelectorAll('input[type="checkbox"]:not(#CheckTodosExamenes)');
  var seleccionados = [];
  ...
  if (seleccionados.length === 0) {
    mensajecentral("error", "Debes seleccionar algún examen.");
  } else {
    let btnimprimir = document.getElementById("imprimirExamnes");
...
// Asignar la función al evento clic del botón "searchCliente"
document.getElementById("searchCliente").addEventListener("click", buscarEmpresa);
...
//función para marcar o desmarcar todas las casillas de los pacientes
function CheckTodos() {
...
function CheckTodosExamenes() {
  let checkdefault = document.getElementById("CheckTodosExamenes");
  if (checkdefault.checked) {
    $('#examenesmodal input[type="checkbox"]').prop('checked', true);
  } else {
    $('#examenesmodal input[type="checkbox"]').prop('checked', false);
  }
}
