    ...
    $.ajax({
        ...
        success: function (pruebas) {
            ...
            pruebas.forEach(prueba => {
                cards.append(`
                        <div id="card-${prueba.codpru_id}" class="card text-white mb-4" style="background-color: #793B8E; color: white">
                            <div class="card-footer d-flex align-items-center justify-content-between">
                                <a id="prueba-${prueba.codpru_id}" data-nombre = "${prueba.desexadet}" class="small text-white stretched-link" onclick="ObtenerParametros(${prueba.codpru_id},${prueba.nuncom}, '${prueba.desexadet}')" href="#">${prueba.desexadet}</a>
...
function ObtenerParametros(codpru_id, nuncomInput, desexadet) {
    let btnGrabar = document.getElementById('btnGrabar');
    ...
    for (let card of cards) {
        if (card.id === `card-${codpru_id}`) {
            card.style.backgroundColor = '#0FA0A7';
        }
        else {
            card.style.backgroundColor = '#793B8E';
        }
    }
    mostrarDiv("cargaparametros");
    ocultarDiv("detInforme");
    ocultarDiv("divformato");
    $.ajax({
        url: '/procedimientosparametros',
        method: "GET",
        data: {
            codpru_id: codpru_id,
            nuncom: nuncomInput
        },
        success: async function (parametros) {
            await CrearContenedorPDF();
            var contenedor = document.getElementById("detInforme");
            var codigoHTML = `
                <div class="input-group input-group-sm mt-2">
                    <label class="input-group-text" for="codigonombprueba_ref">Nombre de prueba</label>
                    <input type="text" class="form-control form-control-sm" id="nombprueba" value='${desexadet}' placeholder="Ingrese el valor" disabled>
                    <span class="input-group-text" style="width: 35px; color: #773C8D;" for="nombprueba"><i class="fa-solid fa-heart-pulse"></i></span>
                </div>
                <div class="input-group input-group-sm mt-2">
                    <label class="input-group-text" for="codigo_ref">Nro referencia</label>
                    <input type="text" class="form-control form-control-sm" id="codigo_ref" value='${nref}' placeholder="Ingrese el valor" disabled>
                    <span class="input-group-text" style="width: 35px; color: #773C8D;" for="codigo_ref"><i class="fa-solid fa-heart-pulse"></i></span>
                </div>
                <div class="input-group input-group-sm mt-2">
                    <label class="input-group-text" for="det_informe">Detalle del informe</label>
                    <textarea class="form-control form-control-sm" id="det_informe" rows="3" placeholder="Ingrese el valor" required>${parametros[0].det_informe}</textarea>
                    <span class="input-group-text" style="width: 35px; color: #773C8D;" for="det_informe"><i class="fa-regular fa-file"></i></span>
                </div>
                <div class="input-group input-group-sm mt-2">
                    <label class="input-group-text" for="conclusion">Conclusión</label>
                    ...
                    <span class="input-group-text" style="width: 35px; color: #773C8D;" for="conclusion"><i class="fa-solid fa-heart-pulse"></i></span>
                </div>
                `;
...
async function poblarcampos() {
    ...
    $.ajax({
        ...
        success: async function (result) {
            ...
            ajustarAnchoDeLabels('detInforme');
        },
...
function Grabar() {
    ...
    fetch('/pbprocedimientos', {
        ...
        .then(data => {
            ...
            // Actualizar el onclick del enlace correspondiente
            let enlace = document.getElementById(`prueba-${codpru_id}`);
            if (enlace) {
                enlace.setAttribute('onclick', `ObtenerParametros(${codpru_id},${nuncom.value}, '${enlace.dataset.nombre}')`);
                enlace.click();
            }
            btnGrabar.disabled = false;
...
async function CrearContenedorPDF() {
    ocultarDiv('divformato');
    let nuncom = $('#nuncom').val();
    $.ajax({
        url: '/getdatosformatosprocedimientos',
        method: "GET",
        data: {
            nuncom: nuncom,
        },
        success: function (result) {
            ...
            let nombreArchivoFormat = JSON.parse(nombreArchivo);

            let rutasArray = [];
            nombreArchivoFormat.forEach((objrutaArchivo) => {
            ...
            let contenedor = document.getElementById("divformato");
            if (nombreArchivo === '0') {
                ocultarDiv('divformato');
            } else {
                console.log(rutasArray);
                Paginacion(paginasTotalDocument);
                mostrarContenido(rutasArray, contenedorPaginacion, contenedor);
                let ContenidoArrays = mostrarDocumentos(rutasArray, contenedor);
                contenedor.style.height = '100%';
                contenedor.innerHTML = ContenidoArrays[0];
                mostrarDiv('divformato');
            }
        },
        error: function (error) {
            alert('error', error);
...
function mostrarDocumentos(rutaArchivo, contenedor) {
    let ContenidoDocumentos = [];
    contenedor.style.overflow = "auto";
    contenedor.style.maxHeight = "100%"; // Ajusta esto según tus necesidades
    ...
    rutaArchivo.forEach(function (DocObj) {
        ...
        if (tipoExtension === 'jpg' || tipoExtension === 'jpeg' || tipoExtension === 'png') {
            ...
            let baseUrl = window.location.origin;
            let pdfUrl = baseUrl + DocObj + '#zoom=80';
