...
document.getElementById('btnImportarCSV').addEventListener('click', function () {
    document.getElementById('archivos_espiro').click();
});

document.getElementById('archivos_espiro').addEventListener('change', (event) => {
    const archivo = event.target.files[0];
    if (archivo) {
        limpiarTabla();
        importarCSV(archivo);
        // Reinicia el input de tipo file para permitir seleccionar el mismo archivo
        event.target.value = '';
    }
});
function limpiarTabla() {
    $('#tbodyparametros tr').each(function () {
        $(this).find('input[data-column="valor_ref"], input[data-column="mejor_valor"], input[data-column="Mejor_val_porc"], input[data-column="valor_pre1"], input[data-column="valor_pre2"], input[data-column="valor_pre3"]').val('');
    });
}
function importarCSV(archivo) {
    ...
    lector.onload = function (event) {
        ...
        const filas = $('#tbodyparametros tr');
        let filaTabla = 0; // Fila actual de la tabla de tbodyparametros
        lineas.forEach((linea, fila) => {
            // Solo procesar las filas 1, 2, 3, 4, 6 y 7
            if ([0, 1, 2, 3, 5, 6].includes(fila)) {
                const columna = linea.split('|'); // Dividimos cada línea por coma
                let valores;
                console.log(columna[34]);
                if (columna[34] === '"POST"') {
                    console.log(columna[67],columna[68],columna[70],columna[72],columna[73],columna[74]);
                    valores = [
                        columna[67], // teorico
                        columna[68], // pre
                        columna[70], // teoricoporcentaje
                        columna[72], // premichi1
                        columna[73], // premichi2
                        columna[74]  // premichi3
                    ];
                }else{
                    valores = [
                        columna[63], // teorico
                        columna[64], // pre
                        columna[66], // teoricoporcentaje
                        columna[68], // premichi1
                        columna[69], // premichi2
                        columna[70]  // premichi3
                    ];
                }
                console.log(valores);
                for (let j = 0; j <= 6; j++) {
                    const column = $(filas[filaTabla]);
                    const inputs = column.find('input[data-column="valor_ref"], input[data-column="mejor_valor"], input[data-column="Mejor_val_porc"], input[data-column="valor_pre1"], input[data-column="valor_pre2"], input[data-column="valor_pre3"]');
                    $(inputs[j]).val(valores[j]);
                }
                filaTabla++;
            }
        });
