        ...
        success: function (result) {
            if (result[0].mensaje != 'sin datos') {
                ...
                if (result[0].psicologia) {
                    ...
                    $('#conducta_sexual').val(registro.conducta_sexual);
...
function Grabar() {
    ...
    let conducta_sexual = $('#conducta_sexual').val();
...
function getCitasCombo() {
    $.ajax({
        ...
        success: function (lista) {
            let tipresult_id = $('#tipresult_id');
            tipresult_id.html('');            
            lista.forEach(item => {
                let option = `<option value="${item.id}">${item.descripcion}</option>`;
                if (item.tabla === 'tipo_resultado') {
                    tipresult_id.append(option);
                }
            });
