$(document).ready(function () {
    $('input[name="ce_pre21"]').change(function () {
        let fuma = $(this).val();  // Obtener el valor actual del radio button
        if (fuma === 'SI') {
            var input = document.getElementById('ce_pre21_rpta');
            input.classList.remove('is-valid', 'is-invalid');
            mostrarDiv("ce_pre21_rpta");
        } else {
            ocultarDiv("ce_pre21_rpta");
...
function Grabar() {
    let fuma = document.querySelector('input[name="ce_pre21"]:checked').value; //valida si es que esta marcado el radio button como SI, debe ingresar la cantidad en el input si o si
    if (fuma === 'SI') {
        var camposValidos = validarInputs('ce_pre21_rpta');
        if (!camposValidos) {
            return;
        }
    }
