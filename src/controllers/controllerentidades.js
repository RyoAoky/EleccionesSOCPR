...
async function actualizarRutas(pool, doc_adic_id) {
    ...
    if (doc_adic_id !== 0 || doc_adic_id !== '0') {
        ...
        const jsonActualizado = await Promise.all(rutas.map(async (doc) => {
            ...
            try {
                await fs.renameSync(rutaArchivoAntiguo, rutaArchivoNuevo);
            } catch (error) {
...
module.exports = {
    ...
    async getempresas(req, res) {
        ...
        //const codrol = req.user.codrol;
        const pool = await getConnection();
        const empresas = await pool.query(`sp_selEmpresa '${empresa}'`);
    ...
    async postprotocolo(req, res) {
        try {
            const { codemp, nompro, comentarios, tipexa_id, estado, cadcermed, tiemval_cermed, fecvcto_cermed, id, datains, fec_emi, cant_pac, med_id } = req.body;
            ...
            request.input('cadcermed', sql.Char(1), cadcermed);
    ...
    async getpaciente(req, res) {//listar paciente para edicion
        ...
        const pool = await getConnection();
        const response = await pool.query(`sp_selpaciente '${parametro}','${parametro1}'`);
        res.json(response.recordset);
    },
    async getPacienteCombos(req, res) {//llenar los combos de formulario       
        //const codrolUser = req.user.codrol;
        const pool = await getConnection();
        const response = await pool.query(`sp_selCombosPaciente`);
        res.json(response.recordset);
    },
    async getDistrito(req, res) {//Modal de busqueda de Distrito
        ...
        //const codrolUser = req.user.codrol;
        const pool = await getConnection();
        const response = await pool.query(`sp_selDistrito '${parametro}'`);
        res.json(response.recordset);
    },
    async getPais(req, res) {//Modal de busqueda de Pais
        ...
        //const codrolUser = req.user.codrol;
        const pool = await getConnection();
        const response = await pool.query(`sp_selPais '${parametro}'`);
        res.json(response.recordset);
    },
    ...
    async getCitasCombos(req, res) {//llenar los combos de formulario       
        //const codrolUser = req.user.codrol;
        const pool = await getConnection();
        const response = await pool.query(`sp_selCombosCita`);
        res.json(response.recordset);
    },
    async getProtocoloCombos(req, res) {//llenar los combos del formulario en base al protocolo
        ...
        //const codrolUser = req.user.codrol;
        const pool = await getConnection();
        const response = await pool.query(`pa_selcmbProtocolo '${parametro}'`);
        res.json(response.recordset);
    },
    ...
    async getListaCitas(req, res) {//listar las citas
        try {
            let { fecini, fecfin, paciente, parametro3, cli_id, parametro4, parametro5, parametro6 } = req.query;
            const codrolUser = req.user.codrol;
            if (paciente === '') {
                paciente = '%';
            }
            const pool = await getConnection();
            const response = await pool.query(`pa_selCitas '${fecini}','${fecfin}','${paciente}','${parametro3}', '${cli_id}', '${parametro4}','${parametro5}','${parametro6}','${codrolUser}'`);
            res.json(response.recordset);
        } catch (error) {
            console.error(error);
            res.status(500).json({ error: error.message });
    ...
    async postdocadjcita(req, res) {
        let { cita_id, doc_adic_id } = req.body;
        const codrol = req.user.codrol;
        const pool = await getConnection();

        const result = await pool.query(`pa_InsDocadjCitas '${cita_id}','${doc_adic_id}','${codrol}'`);
        if (result.recordset[0].icono === 'success') {
    ...
    async postcerrarcitas(req, res) {
        try {
            const citas_id = req.body.citas_id;
            const pool = await getConnection();
            const response = await pool.query(`pa_UpdCerrarCitas'${citas_id}'`);
            res.json(response.recordset);
        } catch (error) {
            console.error(error);
    ...
    async postcitaAdmin(req, res) {
        const { inputid, cargo_actual, fecing_cargo, area_actual, fecing_area, fecing_empresa, altilab_id, superf_id, tipseg_id, cond_vehiculo, ope_equipo_pesado, 
            envresult_correo, com_info_medica, ent_result_fisico, usa_firma_formatos, res_lugar_trabajo } = req.body;
        const usenam = req.user.usuario;
        const codrolUser = req.user.codrol;
        const result = await sequelize.query(`EXECUTE pa_InsCitaAdmin :inputid, :cargo_actual, :fecing_cargo, :area_actual, :fecing_area, :fecing_empresa, :altilab_id,
            :superf_id, :tipseg_id, :cond_vehiculo, :ope_equipo_pesado, :envresult_correo, :com_info_medica, :ent_result_fisico, :usa_firma_formatos, :res_lugar_trabajo,
            :codrolUser, :usenam`, {
            type: QueryTypes.SELECT,
            replacements: { inputid: inputid, cargo_actual: cargo_actual.toUpperCase(), fecing_cargo: fecing_cargo, area_actual: area_actual.toUpperCase(), fecing_area: fecing_area, 
                fecing_empresa: fecing_empresa, altilab_id: altilab_id, superf_id: superf_id, tipseg_id: tipseg_id, cond_vehiculo: cond_vehiculo, 
                ope_equipo_pesado: ope_equipo_pesado, envresult_correo: envresult_correo, com_info_medica: com_info_medica, ent_result_fisico: ent_result_fisico, 
                usa_firma_formatos: usa_firma_formatos, res_lugar_trabajo: res_lugar_trabajo, codrolUser: codrolUser, usenam: usenam }
        });
        res.json(result);
    },
    /*************************/
    ...
    async postcliente(req, res) {//agregar cliente
        try {
            const { docide, NumDoc, razsoc, actividad_economica, Direccion, cod_ubigeo, contacto, emailcon, celular, telefono, emailmedocu, forpag_id, incfirmmedexa, Incfirpacexa, Inchuepacexa, Incfordatper, incdecjur, Incfirhueforadi, creusucatocu, Encorvctocert, envcorusuexi, notinfmed_medocu, notinfmedpac, piccli, inputid } = req.body;
            const usenam = req.user.usuario;
            const hostname = '';
            const codrolUser = req.user.codrol;
            ...
            await sharp(imagenBinaria)
                ...
                .toFile(rutaSalida);
            const pool = await getConnection();
            const response = await pool.query(`pa_InsCliente '${razsoc.toUpperCase()}','${docide}','${NumDoc}','${Direccion}','${telefono}','${emailcon}','${contacto}','${celular}','${emailmedocu}','${forpag_id}','${incfirmmedexa}','${Incfirpacexa}','${Inchuepacexa}','${Incfordatper}','${incdecjur}','${Incfirhueforadi}','${creusucatocu}','${Encorvctocert}','${envcorusuexi}','${notinfmed_medocu}','${notinfmedpac}','${actividad_economica}', '${inputid}', '${cod_ubigeo}','${codrolUser}', '${usenam}'`);
            res.json(response.recordset);
        } catch (error) {
            res.status(500).json({ error: 'Incorrecto ' + error });
        }
    ...
    async getcliente(req, res) {//listar cliente para edicion
        const { parametro, parametro1 } = req.query;
        const codrolUser = req.user.codrol;
        const pool = await getConnection();
        const response = await pool.query(`pa_selCliente  '${codrolUser}','${parametro}', '${parametro1}'`);
        res.json(response.recordset);
    },
    async deletecliente(req, res) {
        try {
            ...
            const detalleJson = JSON.stringify(seleccionados);
            const codrolUser = req.user.codrol;
            const result = await sequelize.query(`EXECUTE pa_delCliente :codrolUser, :ClientesJson`, {
                //para ver que tipo de consulta es(SELECT, UPDATE, DELETE, INSERT)
                type: QueryTypes.DELETE,
                //para poner parametros dentro del procedure                
                replacements: { ClientesJson: detalleJson, codrolUser: codrolUser }
            });
            res.json(result);
        } catch (error) {
    ...
    /**************************************/
    /*************Rangos*******************/
    async getrangos(req, res) {//listar rangos
        const { parametro, codpru_id, parexa_id } = req.query;
        const pool = await getConnection();
        const response = await pool.query(`pa_SelRangos '${parametro}', '${codpru_id}','${parexa_id}'`);
        res.json(response.recordset);
    },
    async postrangos(req, res) {//agregar rango
        let { inputid, codpru_id, parexa_id, codlabuni_id, sexo_id, rango_ini, rango_fin, edad_inicio, edad_fin, desrango, sta_rango_normal, valor_ref, metodo } = req.body;
        const usenam = req.user.usuario;
        const hostname = '';
        if (rango_ini === '') {
            rango_ini = null;
        ...
        const codrolUser = req.user.codrol;
        const pool = await getConnection();
        const response = await pool.query(`pa_InsRangos ${inputid}, ${codrolUser}, '${usenam}', '${hostname}', ${codpru_id}, ${parexa_id}, ${sexo_id}, ${rango_ini}, ${rango_fin}, ${codlabuni_id}, '${desrango}', '${sta_rango_normal}', '${valor_ref}', '${metodo}', ${edad_inicio}, ${edad_fin}`);
        res.json(response.recordset);
    },
    /**************************************/
    /*************Pruebas Examen*******************/
    async getpruebas(req, res) {//listar pruebas
        let parametro = req.query.parametro;
        const pool = await getConnection();
        const response = await pool.query(`sp_SelPruebas '${parametro}'`);
        res.json(response.recordset);
    },
    /**************************************/
    /**************Parametros examen (parexa_id)****************/
    async getparametros(req, res) {//listar parametros
        let parametro = req.query.parametro;
        let codpru_id = req.query.codpru_id;
        const pool = await getConnection();
        const response = await pool.query(`sp_SelParametros '${parametro}', ${codpru_id}`);

        res.json(response.recordset);
    },
    /**************************************/
    async getunidadesandsexo(req, res) {//listar unidades
        const pool = await getConnection();
        const response = await pool.query(`sp_SelUnidadesandsexo`);
        res.json(response.recordset);
    },
};
