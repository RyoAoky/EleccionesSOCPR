...
module.exports = {
    ...
    async nuevacontrasena(req, res) {
        try {
            const { contrasenaanterior, nuevacontrasena } = req.body;
            console.log("contraseña" + req.user.contraseña);
            const validarContrasena = await helpers.matchPassword(contrasenaanterior, req.user.contraseña);
            const nuevacontrasenaencriptada = await helpers.EncriptarPass(nuevacontrasena);

            if (validarContrasena) {
                const result = await sequelize.query(`EXECUTE pa_UpdCambiarContrasena :nuevacontrasena, :iduser`, {
                    //para ver que tipo de consulta es(SELECT, UPDATE, DELETE, INSERT)
                    type: QueryTypes.SELECT,
                    //para poner parametros dentro del procedure                
                    replacements: { nuevacontrasena: nuevacontrasenaencriptada, iduser: req.user.iduser }
                });
                res.json(result);
            } else {
                res.json("N");
